...
{
	//La clave debe tener digitos, letras mayus y minus y caracteres especiales.
	$tam=strlen($pass);
	//$car_esp_encontrado=false;
	//$caracteres_especiales=explode(",","%,&,@,$,?,^,!,#,(,),=,+,-,{,},[,]");
	if($tam>7 && $tam<16)
	{
		return true;
	}
...
//Comprobar si existe un URL y esta Online.

function comprobar_url($url)
{
	$correcta = explode("/", $url);
	if($correcta[0] == "http:")
	{
		return true;
	}
